...
export default function DocumentAIToolsPanel({ documentId }: DocumentAIToolsPanelProps) {
  ...
  const fetchFlashcards = async () => {
    ...
    try {
      const response = await fetch(`/api/documents/flashcards?documentId=${documentId}`)
      if (response.ok) {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const response = await fetch(`/api/documents/questions?documentId=${documentId}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const handleGenerateFlashcards = async (regenerate: boolean) => {
  ...
  const handleGenerateQuestions = async (regenerate: boolean) => {
    setGeneratingQuestions(true)
    try {
      const response = await fetch('/api/documents/generate/questions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ documentId, count: 20, regenerate }),
      })
    ...
    <Card className="h-full flex flex-col">
      ...
      <CardContent className="flex-1 min-h-0 flex flex-col">
        <Tabs defaultValue="flashcards" className="w-full h-full flex flex-col">
          ...
          <TabsContent value="flashcards" className="flex-1 min-h-0 flex flex-col space-y-4 pt-4 overflow-hidden">
            <div className="space-y-2 flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between flex-shrink-0">
                <p className="text-sm text-gray-600">
                  {flashcards.length > 0
                    ? `${flashcards.length} flashcards generated`
                    : 'Generate flashcards to study key concepts from the document.'}
                </p>
                {flashcards.length > 0 && (
                  <Button
                    variant="outline"
              ...
              {loadingFlashcards ? (
                ...
                <div className="space-y-3 flex-1 flex flex-col overflow-hidden">
                  ...
                  <Link href={`/dashboard/documents/${documentId}/flashcards`} className="flex-shrink-0">
          ...
          <TabsContent value="questions" className="flex-1 min-h-0 flex flex-col space-y-4 pt-4">
            <div className="space-y-2 flex-1 flex flex-col">
              ...
              {loadingQuestions ? (
                ...
                <div className="space-y-3 flex-1 flex flex-col">
                  ...
                  <Link href={`/dashboard/documents/${documentId}/quiz`} className="flex-shrink-0">
                    <Button className="w-full">
          ...
          <TabsContent value="notes" className="flex-1 min-h-0 flex flex-col space-y-4 pt-4">
            <DocumentNotesPanel documentId={documentId} />
          </TabsContent>

          <TabsContent value="summary" className="flex-1 min-h-0 flex flex-col space-y-4 pt-4">
